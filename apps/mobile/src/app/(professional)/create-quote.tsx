...
export default function CreateQuoteScreen() {
  ...
  const laborRef = useRef<TextInput>(null);
  const materialRef = useRef<TextInput>(null);
  const transportRef = useRef<TextInput>(null);
  const durationRef = useRef<TextInput>(null);
  const validDaysRef = useRef<TextInput>(null);
  ...
  const parsedValidDays = validDays.trim() ? Number(validDays) : 0;

  const canSubmit = labor > 0 && !!requestId && parsedValidDays >= 0;
  ...
  }, [canSubmit, total, parsedValidDays, requestId, labor, material, transport, description, estimatedDuration, createQuote]);
        ...
        >
          ...
          <View style={styles.section}>
            ...
            <TextInput
              ...
              keyboardType="numeric"
              returnKeyType="next"
              onSubmitEditing={() => transportRef.current?.focus()}
          ...
          <View style={styles.totalCard}>
            ...
            <Text variant="h1" color={colors.primary}>{formatCurrency(total)}</Text>
          ...
          <View style={styles.section}>
            ...
            <TextInput
              ...
              placeholderTextColor={colors.textTertiary}
              returnKeyType="next"
              onSubmitEditing={() => validDaysRef.current?.focus()}
...
const styles = StyleSheet.create({
  ...
  serviceInfo: { flexDirection: 'row', alignItems: 'center', backgroundColor: colors.surface, borderRadius: radius.md, padding: spacing.lg, gap: spacing.md, ...shadows.sm },
  serviceInfoText: { flex: 1, gap: 2 },
  ...
  totalCard: { backgroundColor: colors.primary, borderRadius: radius.md, padding: spacing.xl, alignItems: 'center', gap: spacing.xs },

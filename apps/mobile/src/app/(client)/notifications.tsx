...
function getDeepLink(notification: Notification): string | null {
  ...
  if (data.conversationId) return `/(client)/conversation?conversationId=${data.conversationId}`;
  if (data.quoteId && data.requestId) return `/(client)/quote-detail?quoteId=${data.quoteId}&requestId=${data.requestId}`;
  if (data.bookingId) return `/(client)/booking-detail?bookingId=${data.bookingId}`;
  if (data.requestId) return `/(client)/request-detail?id=${data.requestId}`;
  return null;
}
